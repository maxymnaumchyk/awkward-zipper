...
def _element_link(target_collection, eventindex, index, key):
    ...
    if _isinstance(target_collection, "dask_awkward.lib.core.Array") != _isinstance(
        ...
        msg = "element linking must be done on two dask_awkward arrays or two awkward arrays not a mix of the two"
...
def _element_link_multiple(events, obj, link_field, with_name=None):
    ...
    unique_keys = [
        i
        for i in np.unique(awkward.to_numpy(awkward.flatten(key, axis=None)))
        if i != 0
    ]
